...
const senderBusinessNameEl = document.getElementById('senderBusinessName');
...
const clientBusinessNameEl = document.getElementById('clientBusinessName');
...
function renderInvoice(invoice) {
    ...
    clientBusinessNameEl.textContent = invoice.clientBusinessName ?? '';
    ...
    senderBusinessNameEl.textContent = invoice.senderBusinessName ?? '';
    ...
    const labor = invoice.hours * invoice.rate || 0;
    const tax = invoice.tax ?? 0;
    const total = invoice.total ?? labor;

  totalsEl.innerHTML = `
    <p>Hours: ${invoice.hours || 0}</p>
    <p>Rate: $${invoice.rate || 0}</p>
    <p>Subtotal: $${labor.toFixed(2)}</p>
    ${invoice.taxExempt ? '' : `<p>Tax: $${tax.toFixed(2)}</p>`}
